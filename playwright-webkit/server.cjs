...
const executablePath = process.env.PW_WEBKIT_EXECUTABLE_PATH || "";
...
async function main() {
  ...
  console.log(
    `Playwright WebKit server listening at ${server.wsEndpoint()} (headless=${headless}, display=${process.env.DISPLAY || "unset"})`,
  );
...
main().catch((error) => {
  console.error("Failed to start Playwright WebKit server:", error);
  process.exit(1);
});
